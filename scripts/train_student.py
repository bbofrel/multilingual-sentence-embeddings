...
def encoding_english_sentences(config, version='full'):
    """ encodes english sentences from a parallel dataset using the teacher model """
...
def dataloader_creation(english_sentences, german_sentences, teacher_embeddings, config, shuffle=True):
    """ creates a PyTorch DataLoader from english and german sentences and teacher embeddings """
...
def prepare_dataset(config):
    """ prepares train and development DataLoaders for student model training """
...
def training_loop(config, train_loader, dev_loader):
    ...
    BATCH_SIZE = config['training_args']['batch_size']
    KEY = config['training_args']['wandb_key']
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        torch.save(student_model.state_dict(), "models/checkpoints/student_xlmr_distilled.pt")
